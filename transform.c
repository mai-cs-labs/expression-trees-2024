    ...
    static bool factor_difference_of_cubes(Expression* const expression);
    ...
    void expand_expression(Expression* const expression)
    {
        ...
        factor_difference_of_cubes(expression);
    }
    ...
    *
    ...
    static bool factor_difference_of_cubes(Expression* const expression)
    {
        ...
        switch (expression->type) {
            // Try to find difference of cubes in unary expression
            case ExpressionType_Unary: {
                UnaryExpression* const unary = (UnaryExpression*)expression;
                return factor_difference_of_cubes(unary->subexpression);
            } break;

            case ExpressionType_Binary: {
                BinaryExpression* const binary = (BinaryExpression*)expression;

                // Try to find difference of cubes in the sides of binary expression first
                const bool other_result = factor_difference_of_cubes(binary->left) |
                                          factor_difference_of_cubes(binary->right);

                // First step, find _difference_ of two expressions
                if (binary->operator == TokenType_Minus) {
                    Expression* a = NULL;
                    Expression* b = NULL;
                    bool lhs_pow_of_three = false;
                    bool rhs_pow_of_three = false;
                    ...
                    if (binary->left->type == ExpressionType_Binary) {
                        ...
                        if (lhs->operator == TokenType_Exponent) {
                            ...
                            if (lhs->right->type == ExpressionType_Literal) {
                                ...
                                // specifically, number literal
                                if (number->tag == LiteralTag_Number) {
                                    // specifically, with value of 3
                                    if (fabs(number->number - 3.0) < DBL_EPSILON) {
                                        lhs_pow_of_three = true;
                                        a = lhs->left;
                                    }
                                }
                            }
                    ...
                    // This is true when left and right differences
                    // are expressions "raise to power of three" respectively
                    if (lhs_pow_of_three && rhs_pow_of_three) {
                        // Change operator from subtraction to multiplication
                        binary->operator = TokenType_Multiply;
                        a->parenthesised = false;
                        b->parenthesised = false;
                        ...
                        // A - B subexpression must be parenthesised
                        new_lhs->base.parenthesised = true;
                        a->parenthesised = true;
                        b->parenthesised = true;
                        // Create new (A^2 + AB + B^2) expression
                        BinaryExpression* term1 = expression_binary_create(
                                TokenType_Exponent, expression_copy(a),
                                (Expression*)expression_literal_create_number(2.0));
                        term1->base.parenthesised = false;

                        BinaryExpression* term2 = expression_binary_create(
                                TokenType_Multiply, expression_copy(a), expression_copy(b));

                        BinaryExpression* term3 = expression_binary_create(
                                TokenType_Exponent, expression_copy(b),
                                (Expression*)expression_literal_create_number(2.0));
                        term3->base.parenthesised = false;

                        BinaryExpression* new_rhs = expression_binary_create(
                                TokenType_Plus,
                                (Expression*)expression_binary_create(TokenType_Plus, (Expression*)term1, (Expression*)term2),
                                (Expression*)term3
                        );
                        // A^2 + AB + B^2 subexpression must be parenthesised
                        new_rhs->base.parenthesised = true;
                ...
                // Maybe the differences of cubes were found deeper in the expression?
                return other_result;
            } break;
        }
